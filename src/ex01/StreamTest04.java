...
public class StreamTest04 {
	...
	public static void main(String[] args) {
		...
		// 3) 전체 합계
		// mapToInt : Stream 안에 있는 내용을 하나씩 꺼네서 변경 작업
		...
		List<Integer> evenList = list.stream()
				.filter( n ->  n % 2 == 0 )
				.toList();
		System.out.println("작수 목록:" + evenList );
		
		// 5)  map : 모든 데이터를  10배로 변환
		List<Integer> mappedList = list.stream()
				.map( n -> n * 10  )
				.toList();   // stream list -> arraylist 변환
		...
		double result = list.stream()
		    .filter(n -> n >= 3)
		    .map( n -> n * n )
		    .mapToDouble( Integer::doubleValue )   // sum() 할려고 Integer -> int 로 변환
		    .average()
		    .orElse(0.0);   
		     // OptionalDouble  : 결과가 null 될수없다
		     // 만약 null 이면   .orElse(0.0) 추가해서 0.0 으로 만들어주어야한다
		   // .sum();    // 결과는 int
		System.out.println("최종결과" + result );
		
		// 7) forEach
		// Stream<String> stream = Stream.of("apple","banana", "cherry", "durian");
		String []     fruits    = {  "cherry", "durian", "apple","banana", "apple" };
